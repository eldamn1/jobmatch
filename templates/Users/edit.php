...
<html>
<head>
    <title>Edit User | JobMatch</title>
...
<div class="content" id="content">
    ...
    <div class="dash-panel" style="max-width:720px;">
        ...
        <div class="form-grid">
            <div class="form-row"><label>Full Name</label><?= $this->Form->text('fullname', ['required' => true]) ?></div>
            <div class="form-row"><label>Email</label><?= $this->Form->email('email', ['required' => true]) ?></div>
            <div class="form-row"><label>New Password</label><?= $this->Form->password('password', ['value' => '']) ?><div class="form-hint">Leave blank to keep current password.</div></div>
            <div class="form-row"><label>Role</label>
                <select name="user_group_id">
                    <option value="1" <?= ((int)$user->role_id === 1) ? 'selected' : '' ?>>Admin</option>
                    <option value="2" <?= ((int)$user->role_id !== 1) ? 'selected' : '' ?>>User</option>
                </select>
            </div>
            <div class="form-row"><label>Status</label>
                <select name="status">
                    <option value="1" <?= ((string)$user->status === '1') ? 'selected' : '' ?>>Active</option>
                    <option value="0" <?= ((string)$user->status !== '1') ? 'selected' : '' ?>>Inactive</option>
                </select>
            </div>
            <div class="form-row"><label>Avatar</label>
                <?php if (!empty($user->avatar)): ?>
                    <div style="margin-bottom:8px;">
                        <img src="<?= $this->Url->image('avatars/' . $user->avatar) ?>" alt="avatar" style="height:56px;width:56px;border-radius:50%;object-fit:cover;">
                        <label style="display:inline; text-transform:none; font-weight:400; margin-left:8px;"><input type="checkbox" name="delete_avatar" value="1"> Remove avatar</label>
                    </div>
                <?php endif; ?>
        ...
        <div class="form-actions">
            <button type="submit" class="btn-gold"><i class="fa-solid fa-floppy-disk"></i> Update</button>
            <?= $this->Html->link('Cancel', ['action' => 'index'], ['class' => 'btn-outline-nav']) ?>
        </div>
        <?= $this->Form->end() ?>
    </div>
</div>
